...
def add_probe_SHARPTrack(probe_locations_path, probe_border_table_path):
    borders_table = read_borders_table(probe_border_table_path)
    probe_locs = read_probe_mat(probe_locations_path)    s

    
    actor = Cylinder2(StereoToCCF(cranial_site_xyz), StereoToCCF(target_site), scene.root, color='black', radius=radius)
    scene.add(actor)

    cranial_site_xyz= np.array([0, 0, 0])
    depth=1000
    attack_angle_x=-15
    color="red"
    distance_between_probes=250
    radius=20, 
    mark_true=False
    # presumes the only angle that varies is the y axis of the stereotax (What we call Y but is called X here)
...
add_probe_SHARPTrack(probe_locations_path='/Users/harryclark/Documents/probe_data/M20_probe_locations_1.mat',
                     probe_border_table_path='/Users/harryclark/Documents/probe_data/M20_probe_border_table_1.mat')
